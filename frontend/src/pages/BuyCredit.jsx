...
const BuyCredit = () => {
 ...
 const paymentStripe = async (planId) => {
  try {
    // 1. Check Clerk's status BEFORE calling openSignIn
    if (!isSignedIn) {
      // ONLY call this if the user is actually logged out
      return openSignIn(); 
    }

    // 2. If the code reaches here, the user IS signed in.
    // Proceed with your Stripe logic...
    setLoading(planId);
    ...
    if (data.success) {
      navigate('/payment', { state: { clientSecret: data.clientSecret, planId } });
    }
  } catch (error) {
    console.error(error);
